...
model_obj = readRDS('RDM4_hier_fit.RData')
load('RDM4_hier_data.RData')
...
sim_data = function(dataList, v1, v2, v3, a, theta, rating_raw) {
  # v1, v2, a = vector[N]
  ...
  for (i in 1:5){
    v_tmp = v1[dataList$subj] + v2[dataList$subj] * x_v[,i] +
      v3[dataList$subj] * dataList$x_strength
    v_trial[,i] = exp(dwell[,i] * v_tmp  + 
                        theta[dataList$subj] * (1- dwell[,i]) * v_tmp)
  }
...
save(ppd, file = "RDM4_hier_ppd.RData")
