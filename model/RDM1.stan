...
transformed parameters {
  ...
  // define drift rate with subject-specific parameter
  for (s in 1:S){
    // chosen rating
    v_chosen[subj_start[s]:subj_end[s]] = exp(v_beta[1,s] + v_beta[2,s] * x_chosen[subj_start[s]:subj_end[s]]);
    // unchosen rating
    for (i in 1:(Kx-1)){
      v_unchosen[subj_start[s]:subj_end[s],i] = exp(v_beta[1,s] + v_beta[2,s] * x_unchosen[subj_start[s]:subj_end[s],i]);
    }
  }
}
