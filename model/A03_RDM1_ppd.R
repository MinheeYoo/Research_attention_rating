...
model_obj = readRDS('RDM1_hier_fit.RData')
load('RDM1_hier_data.RData')
...
sim_data = function(dataList, v1, v2, a, rating_raw) {
  # v1, v2, a = vector[N]
  ...
  for (i in 1:5){
    v_trial[,i] = exp(v1[dataList$subj] + v2[dataList$subj] * x_v[,i])
  }
...
save(ppd, file = "RDM1_hier_ppd.RData")
