...
transformed parameters {
  ...
  for (i in 1:Kv) v_beta[i, ] = v_beta_mu[i] + v_beta_sd[i] * v_beta_raw[i]; // v_beta_raw[i] = ith row
   ...
   // theta = attentional discount 
  theta = theta_mu_raw + theta_sd * theta_raw; 
  
  // define drift rate with subject-specific parameter
  for (s in 1:S){
    // chosen rating
    v_chosen[subj_start[s]:subj_end[s]] = 
        exp(v_beta[1,s] 
            + v_beta[2,s] * x_chosen[subj_start[s]:subj_end[s]]
            + v_beta[3,s] * x_strength[subj_start[s]:subj_end[s]]
            + theta[s] * d_chosen[subj_start[s]:subj_end[s]]);
            
    // unchosen rating
    for (i in 1:(Kx-1)){
      v_unchosen[subj_start[s]:subj_end[s],i] = 
        exp(v_beta[1,s] 
            + v_beta[2,s] * x_unchosen[subj_start[s]:subj_end[s],i] 
            + v_beta[3,s] * x_strength[subj_start[s]:subj_end[s]] 
            + theta[s] * d_unchosen[subj_start[s]:subj_end[s],i]);
    }
  }
}
