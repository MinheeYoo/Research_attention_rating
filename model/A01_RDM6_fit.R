...
makeDwellMatrix = function(df) {
  # df = [rating_p1, dwell time for rating 1:10]
  # return the dwell time of chosen rating in the first column
  # and dwell time for the remaining ratings in 2-5th column 
  # order of the column is the same as the rating matrix 
  rating_p1 = df[1]
  dwellTime = df[2:length(df)]
  
  if (rating_p1 %% 2 == 1) { # odd numbers
    ratingVec = c(1,3,5,7,9)
  } else if (rating_p1 %% 2 == 0) {
    ratingVec = c(2,4,6,8,10)}
  rating_idx = c(rating_p1, ratingVec[ratingVec != rating_p1])
  
  return(dwellTime[rating_idx])
}
...
behavEye2 = behavEye2 %>% filter(id != 12)
...
# dwell time 
tmp = behavEye2 %>% 
  filter(phase == 1, roiID != 99) %>%
  group_by(id, trial, rating, roiID) %>%
  reframe(roiDwell = sum(dwellTime)) %>%
  group_by(id, trial) %>%
  mutate(totalDwell = sum(roiDwell),
         fracDwell = roiDwell / totalDwell) %>% 
  ungroup() %>%
  select(id, trial, rating, roiID, fracDwell) %>% 
  pivot_wider(names_from = roiID, values_from = fracDwell, 
              values_fill = 0) %>%
  select(id, trial, rating, sprintf("%d", 1:10)) %>%
  arrange(id, trial)

# First column = dwell time for chosen rating 
# second-fith = dwell time for the remaining ratings
dwellMatrix = t(apply(as.matrix(tmp[, 3:13]), 1, makeDwellMatrix))
...
dataList = list(
  ...
  x_strength = abs(data_wide$rating_p2 - 5.5)/10,
  d_chosen = dwellMatrix[,1], 
  d_unchosen = dwellMatrix[,2:5],
...
# stan_model = stan_model("RDM6.stan")
# saveRDS(stan_model, "RDM6_compiled.RData")
stan_model = readRDS("RDM6_compiled.RData")
...
initf <- function() {
  list(a_mu_raw = log(2), 
       ...
       theta_mu_raw = 0, 
       theta_raw = runif(S, -0.1, 0.1), 
       theta = runif(S, -0.1, 0.1))
}
...
model_obj = rstan::sampling(stan_model,
                            ...
                            pars = c("a_mu_raw", "a_sd", "a",
                                     ...
                                     "log_lik", "dev"), 
                            init = initf,
                            warmup = 1000, iter = 3000, chains = nChains, cores = nChains, 
                            control=list(adapt_delta=0.99,
                                         stepsize = 0.01, max_treedepth = 15))

save(dataList, data_wide, file = "RDM6_hier_data.RData")
saveRDS(model_obj, "RDM6_hier_fit.RData")
